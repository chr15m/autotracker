...
def brk():
    return samples + "/" + random.choice(["think.wav", "amen.wav"])

def bleep(t):
    return samples + "/" + random.choice([f for f in os.listdir(samples) if f.startswith("c64") and "-" + t + ".wav" in f])

SMP_KICK = itf.smp_add(Sample_FileSlice(filename = brk(), which=0))
SMP_HHC = itf.smp_add(Sample_FileSlice(filename = brk(), which=5))
SMP_HHO = itf.smp_add(Sample_FileSlice(filename = brk(), which=5))
SMP_SNARE = itf.smp_add(Sample_FileSlice(filename = brk(), which=4))
